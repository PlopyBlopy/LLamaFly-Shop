...
public static class DistributedCacheExtensions
{
    ...
    public static ValueTask<Result<T>> GetResultAsync<T>(this IDistributedCache cache, string key, Func<CancellationToken, ValueTask<Result<T>>> getMethod, DistributedCacheEntryOptions? options = null, CancellationToken cancellation = default)
    => GetResultAsyncShared<int, T>(cache, key, state: 0, (_, ct) => getMethod(ct), options, cancellation);
    ...
    private static async ValueTask<Result<T>> GetResultAsyncShared<TState, T>(
    IDistributedCache cache,
    string key,
    TState state,
    Func<TState, CancellationToken, ValueTask<Result<T>>> getMethod,
    DistributedCacheEntryOptions? options,
    CancellationToken cancellation)
    {
        var cachedBytes = await cache.GetAsync(key, cancellation).ConfigureAwait(false);
        if (cachedBytes != null)
        {
            try
            {
            ...
            {
                // Игнорируем некорректные данные в кэше
            }
        }

        var result = await getMethod(state, cancellation).ConfigureAwait(false);

        if (result.IsSuccess)
        {
            var bytes = Serialize(result.Value);
            await (options != null
                ? cache.SetAsync(key, bytes, options, cancellation)
                : cache.SetAsync(key, bytes, cancellation)).ConfigureAwait(false);
        }
